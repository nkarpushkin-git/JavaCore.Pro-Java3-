...
public class ClientHandler {
    ...
    public static final Logger userLogger = Logger.getLogger("");
    ...
    public ClientHandler(Server server, Socket socket) {
        ...
        try {
            handler = new FileHandler("AuthLog.log", true);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        userLogger.getHandlers()[0].setLevel(Level.INFO);
        userLogger.getHandlers()[0].setFormatter(new SimpleFormatter());
        Handler userHandler = null;
        try {
            userHandler = new FileHandler("UsersLog.log", true);
        } catch (IOException e) {
            e.printStackTrace();
        }
                        ...
                        if (str.startsWith("/auth")) { // /auth login72 pass72
                            ...
                            if (newNick != null) {
                                if (!server.isNickBusy(newNick)) {
                                    ...
                                    logger.log(Level.INFO, "user " + nick + " logged successfuly");
                                    break;
                                } else {
                                    sendMsg("Учетная запись уже используется");
                                    AuthService.addLoginLog(tokens[1], "loginBusy");
                                    userLogger.log(Level.INFO, "user: " + nick + " login failed, reason: login busy");
                                }
                            } else {
                                sendMsg("Неверный логин/пароль");
                                AuthService.addLoginLog(tokens[1], "wrongCredentials");
                                userLogger.log(Level.INFO, "user: " + nick + " login failed, reason: wrong credentials");
                            }
                        }
                    }
                    while (true) {
...
//                            String newNUser = AuthService.getNickByLoginAndPass(tokens[1], tokens[2]);
                            ...
                            userLogger.log(Level.INFO, "created new user: " + nick);
                            break;
//                            if (newNUser != null) {
                    ...
                    while (true) {
                        ...
                        if (str.startsWith("/")) {
                            if (str.startsWith("/end")) {
                                ...
                                userLogger.log(Level.INFO, "user: " + nick + " login Out");
                                break;
                            }
                            if (str.startsWith("/w ")) { // /w nick3 lsdfhldf sdkfjhsdf wkerhwr
                                String[] tokens = str.split(" ", 3);
                                String m = str.substring(tokens[1].length() + 4);
                                userLogger.log(Level.INFO, "user: " + this + " sends private message to user: " + tokens[1]);
                            ...
                            if (str.startsWith("/blacklist ")) { // /blacklist nick3
                                ...
                                userLogger.log(Level.INFO, "user: " + this + " adds user: " + tokens[1] + " to blacklist");
                            }
                            if (str.startsWith("/undoblacklist ")) { // /blacklist nick3
                                ...
                                userLogger.log(Level.INFO, "user: " + this + " removes user: " + tokens[1] + " from blacklist");
                            }
                        } else {
                            server.broadcastMsg(this, nick + ": " + str);
                            userLogger.log(Level.INFO, "user: " + nick + " sends broadcast message");
                        }
